...
class ActionVar(Var):
    ...
    def update_actions(self, update_action_idxs: list, list_params: list, update_state_window: int, device: torch.device):
        ...
        for count,idx in enumerate(update_action_idxs):
            ...
            #Prepare and execute Objective. Make sure to SHOW the logging info from trainer.
...
if __name__ == '__main__':
    ...
    class Objective:
        ...
        def execute_objective(self, turn_off_training_log: bool):
...
class ConfigSpace(ABC):
    ...
    def __init__(self, init_config=None):
        ...
        invalid_source_format = {
            ...
            if not isinstance(source, dict) or not set(source.keys()).issubset({'params','sample','save_state'})
        }
        if invalid_source_format:
            raise ValueError(f"All values in 'init_config' must be a dictionary with keys ['params', 'sample', 'save_state'(optional)] but following found: {invalid_source_format}")
    ...
    def verify_objective(self, missing_var_types_raise_error: bool) -> tuple:
        """
    ...
    def _verify_missing_sig_var_types(self, missing_sig_var_types_raise_error: bool):
        ...
        for key, source in self._config.items():
            ...
            if '--action' in var_type and belongs_to is None:
                ...
                #sig_var_types <- Union of all signature(func) for all func in 'choices'.
                sig_var_types = reduce(set.union, [set(signature(func).parameters.keys()) - {'device'} for func in source['params']['choices']], set())
                #Below segment checks for missing parameters. We don't output an error immediately.
                missing_var_types = sig_var_types - self.sig_var_types(action_var_type)
    ...
    def action_var_types(self, save_state: bool = None) -> set:
        ...
        return {var_type[:-len('--action')] for (var_type, _), source in self._config.items()
                if '--action' in var_type and source['save_state'] == save_state}
    ...
    def get_sources(self, list_of_keys) -> list:
        ...
        return [self.get_source(parameter=key[0], belongs_to=key[1]) for key in list_of_keys]
    ...
    def get_var(self, var_type: str, belongs_to: str = None) -> Var:
        ...
        cls = ModelAction if source['save_state'] else Action
        action_choices = []
        for action_func in choices:
            sig = set(signature(action_func).parameters.keys()) - {'device'}
            missing_var_types = sig - self.sig_var_types(action_var_type)
            if missing_var_types:
            ...
            action_choices.append(cls(action_func, sig_dict))
        source = {
    ...
    def set_categorical(self, var_type: str, choices: list, belongs_to: str = None, action: bool = False, save_state: bool = False):
        ...
        if not action or (var_type+'--action', belongs_to) not in self._config.keys():
            ...
            self.set_categorical(var_type=var_type, choices=choices, action=action, save_state=save_state)
            ...
            inherit_sig_var_keys = {
                (sig_var_type,var_type) for sig_var_type in sig_var_types
...
class TestUserConfigSpace(unittest.TestCase):
    ...
    def test_verify_objective(self):
        with self.assertRaises(ValueError):
            self.cs.verify_objective(True)
        self.cs.verify_objective(False)
        self.assertTrue(True) #If it passes up to this point.
...
if __name__ == '__main__':
    ...
    class Objective:
        ...
        def execute_objective(self, turn_off_training_log):
            model = self.default_dict['model_action'](device=self.device)
...
class TestObjective(unittest.TestCase):
    ...
    def test_execute_objective_trial(self):
        ...
        objective_loss, model_container = objective.execute_objective(False)
...
class Auto:
    def __init__(self, INIT_CONFIG: ConfigSpace, storage_dir: str = '/content/trial_storage/'):
        ...
        self.init_config.verify_syntax(missing_sig_var_types_raise_error=False)
        self.init_config.verify_objective(missing_var_types_raise_error=False)
    ...
    def optimize(self, n_auto: int, top_k: int, update_state_window: int, device: torch.device):
        ...
        try:
            ...
            raise RuntimeError(f"Following unexpected error is raised: {e}")
